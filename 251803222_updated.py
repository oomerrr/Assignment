...
def loginpage():                                                                      #function for the login page
    users=["user","admin"]
...
def display_seats(seats):                                                         ##function to display seating pattern
...
def add_flight():                                                                ##function to add a flight
    flight_name = input("Enter the flight name: ")
    if flight_name in flights:
        print("Flight already exists.")
        more_options2()
    
    rows = int(input("Enter the number of rows: "))                             ##taking no. of rows and columns for seating pattern
    ...
    departure_date = input("Enter departure date: ")                           ###taking other flight information
...
def modify_flight():
    ...
    if flight_name in flights:                                            #displaying modification options
        ...
        if choice == "1":
            new_departure_date = input("Enter new departure date: ")         ###choices created corresponding to numbers
...
def remove_flight():                                      ###function to remove flights
    if len(flights)<1:
...
def book_flight():                                       ##function to book flights
    if len(flights)<1:
        ...
        while row_choice < 1 or row_choice > rows:
           print("Invalid row  number.")                                              ###booking flights
           row_choice = int(input("Enter the row number you want to book: "))
...
def cancel_flight():
    if len(flights)<1:
        ...
        while row_choice < 1 or row_choice > rows:                                       ##cancelling flights
...
def show_flights():
    if len(flights) < 1:
        print("NO FLIGHTS AVAILABLE")                                            ###displaying available flights
    else:
        print("Available flights:")
        for flight_name in flights:
            print(f"- {flight_name}")           
...
def load_flight_data():                                                     
    try:
        with open("flights.txt", "r") as file:
            for line in file:
                data = line.strip().split('/')           ###data is a dictionary formed of the data in file
                flight_name = data[0]
                row = int(data[1])
                column = int(data[2])
                date = (data[3])
                ...
                for i in range(6, len(data)):                                               ###row and col of booked seats
                    seat_info = data[i].split('-')
                    row_num = int(seat_info[0])
                    col_num = int(seat_info[1])
                    seats[row_num - 1][col_num - 1] = 'X'
                
                flights[flight_name] = {'seats': seats, 'rows': row, 'columns': column,"departure_date": date, "arrival_time": arrival, "departure_time": departure}

    except FileNotFoundError:
        pass

def save_flight_data():
    with open("flights.txt", "a") as file:               ## apending flight data to files
        for flight_name, data in flights.items():        ##flights.item acessing the elements in flights dictionary   ###data dictionary being created
            rows = data['rows']
            ...
            for row in range(rows):
                for col in range(columns):             ###loop to find booked seats
                    if seats[row][col] == 'X':
                        file.write(f"/{row+1}-{col+1}")
            file.write("\n")

def save_ticket(name, booker, row, column):
    with open("tickets.txt", "a") as file:            ###saving booking details
